...
import { X, Clock, Coins, Loader2 } from 'lucide-vue-next'
...
async function handleRent(): Promise<void> {
  if (!selectedOption.value || !auth.user || !canAfford.value) return
  ...
  try {
    ...
    let fromPurchased = Math.min(auth.profile!.purchased_credits, creditsToSpend)
    let fromSubscription = creditsToSpend - fromPurchased
